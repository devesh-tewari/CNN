...
def three_layer_convnet(x, params):
    ...
    N = x.shape[0]  # Number of images in a batch
    
    channel_1 = conv_w1.shape[3]
    channel_2 = conv_w2.shape[3]
    
    KW1, KH1 = conv_w1.shape[0], conv_w1.shape[1]
    KW2, KH2 = conv_w2.shape[0], conv_w2.shape[1]
    
    C1 = tf.nn.conv2d(x, conv_w1, [1,1,1,1], padding='SAME')
    C1 = tf.nn.bias_add(C1, conv_b1)
    
    C1 = tf.nn.relu(C1)
    
    C2 = tf.nn.conv2d(C1, conv_w2, [1,1,1,1], padding='SAME')
    C2 = tf.nn.bias_add(C2, conv_b2)
    
    C2 = tf.nn.relu(C2)
    
    C2 = tf.reshape(C2, [-1, fc_w.get_shape()[0]])
    
    scores = tf.add( tf.matmul(C2, fc_w), fc_b )
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        self.C1 = tf.layers.Conv2D(filters=channel_1, kernel_size=5, strides=1, 
                                   padding='same', activation=tf.nn.relu,
                                   use_bias=True)
        
        self.C2 = tf.layers.Conv2D(filters=channel_2, kernel_size=3, strides=1, 
                                   padding='same', activation=tf.nn.relu,
                                   use_bias=True)
        
        self.FC1 = tf.layers.Dense(num_classes, activation=tf.nn.relu,
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
                tf.keras.layers.Conv2D(16, kernel_size=5, activation=tf.nn.relu, input_shape=input_shape,
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
                tf.keras.layers.Conv2D(32, kernel_size=5, activation=tf.nn.relu, input_shape=input_shape,
                                         padding='same', use_bias=True, kernel_initializer=initializer),
    ...
    net = tf.keras.Sequential(layers)(inputs)
...
# #### Part 1
# In part 1, we used 'tf.nn.conv2d' function to describe the convolutional layers and 'tf.nn.bias_add' to add bias to it. <br>
# 'tf.nn.relu' was used to apply the relu activation function. <br>
# Finally 'tf.matmul' and 'tf.add' functions were used for mutiplying with dense weights and adding the bias. <br>
# Then to initialize the weights and biases, 'tf.Variable' and 'kaiming_normal' functions are used.
# <br>
# 
# #### Part 2
# In this part we used the initializer 'tf.variance_scaling_initializer' with scale 2. 'tf.layers.Conv2D' function is used to describe the convolutional layers. 'tf.layers.Dense' is used to describe the full connection layer. <br>
# Then in the call method, we propagate the input though each layer of the network and return the output.
# <br>
# In the 'model_init_fn' we simply set the model to 'ThreeLayerConvNet(channel_1, channel_2, num_classes)'.
# <br>
# 'MomentumOptimizer' is used as an optimizer function.
# <br>
# 
# #### Part 3
# We used the variance scaling initializer to initialize weights with scale 2.
# <br>
# I created a list of layers and passed it to 'tf.keras.Sequential' to define the model.
# <br>
# 'MomentumOptimizer' is used as an optimizer function.
# <br>
# 
# 
# #### Part 4
# We used the tensorflow sequential API to implement this part. <br>
# We used the variance scaling initializer to initialize weights with scale 2. <br>
# I created a list of layers and passed it to 'tf.keras.Sequential' to define the model.
# <br>
# The list includes two convolution layers with filter numbers 32, 64 respectively with filter size of 5. <br>
# After each convolution layer, max pooling is used with pool size 2 and pool stride 2. <br>
# Then we flatten the data and then add a dense layer.
# <br>
# 'GradientDescentOptimizer' is used as the optimization function with learning rate 0.01
